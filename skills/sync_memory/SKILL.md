...
2. AskUserQuestionで**カテゴリごとの内訳を見せつつ**対応方針を聞く
3. ユーザーの選択に応じて処理する

**AskUserQuestionのquestion欄にカテゴリ内訳を含める:**

questionのテキスト内で、カテゴリごとにタスクを一覧表示する:
```
pendingタスクの棚卸し: 以下を確認して対応方針を選んでね
...
- **全部完了にして作り直す**: 該当タスクを全て `completed` にし、最新の文脈で `add_task` で新規作成する
- **完了済み・陳腐化のみ処理**: 明らかに完了or陳腐化したタスクだけ `completed` にし、まだ有効なタスクはそのまま残す
- **そのまま残す**: 全部pendingのまま維持

**「全部完了にして作り直す」が選ばれた場合:**
- 該当タスクを全て `update_task_status(task_id, "completed")` で完了にする
- 各タスクについて最新の文脈でdescriptionを書き直し、`add_task` で新規作成する
