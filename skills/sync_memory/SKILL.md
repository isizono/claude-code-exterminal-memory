...
ステップ0で取得したpendingタスク一覧を精査する。**該当タスクがなければこのステップ全体をサイレントスキップする。**
...
2. **どのカテゴリにも該当するタスクがなければ、ステップ6をサイレントスキップする**
3. 該当タスクがある場合、1回のAskUserQuestionで以下の2セクションにまとめて確認する
...
- 該当タスクがないセクションは省略する（片方のセクションだけでもOK）
- 判断に迷うものは載せない（誤って消すより残すほうが安全）

**ユーザーの回答に応じて処理:**
- completedセクション: ユーザーがOKすれば `update_task_status(task_id, "completed")` で更新
