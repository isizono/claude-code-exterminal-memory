...
このツール群は、過去の会話コンテキスト（トピック・決定事項・ログ・アクティビティ）の取得・記録と、
タグによる整理を行います。
...
記録時には必ずタグを付けてください。`domain:`タグは必須です。
namespace: `domain:`（プロジェクト）/ `scope:`（作業の塊）/ `mode:`（作業スタンス）/ 素タグ（キーワード）
...
tag notesに書くもの: そのタグの文脈でしか使わない教訓・注意点。
横断的な知見はauto memory（MEMORY.md）へ。
